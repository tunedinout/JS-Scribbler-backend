...
async function UpdateScribblerFolder(accessToken, scribblerName, scribblerFolderId) {
  ...
  try {
    const existingFolderId = await getFolderIdByName(accessToken, scribblerName, scribblerFolderId)
    log('existingFolderId', existingFolderId)
    let newFileId
    if (!existingFolderId) {
      const folderMetadata = {
 ...
 * @returns
 */
async function saveFileToGoogleDrive({ filename = '', data: fileData }, accessToken, folderId) {
  /**
...
async function validateUserSession(userId) {
  const log = logger(
    ...[loggingContext.googleApisUtils.validateUserSession, getCallerFunctionName()]
  )
  try {
