...
async function postScribble(req, res) {
  try {
    ...
    if (!userId) res.status(401).send({ message: 'unauthorized' })
      ...
      const scribbleId = await UpdateScribblerFolder(accessToken, name, driveFolderId)

      // create default js file
      await saveFileToGoogleDrive({ filename: 'index.js', data: js }, accessToken, scribbleId)

      // create the default html file
      await saveFileToGoogleDrive({ filename: 'index.html', data: html }, accessToken, scribbleId)

      // create the default css file
      await saveFileToGoogleDrive({ filename: 'index.css', data: css }, accessToken, scribbleId)
